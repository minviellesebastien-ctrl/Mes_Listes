...
/* Liste des catégories fixes qui conservent le mode classique */
const CLASSIC_LISTS = ['Liste Cadeaux', 'Liste Idées', 'Liste Courses', 'À Faire'];
...
function addDiverseList() {
    ...
    if (name) {
        const allFixed = ['Mes Repas', 'Liste Cadeaux', 'Liste Idées', 'Liste Courses', 'À Faire'];
        ...
        if (!appData.items[name]) {
            appData.items[name] = ''; /* Les nouvelles listes sont créées en zone de texte libre */
        }
        input.value = '';
        renderDiverseLists();
    }
...
/* Ouverture adaptative des pop-ups */
function openModal(title) {
    currentActiveList = title;

    // Mode classique avec champ de saisie + bouton ajouter
    if (CLASSIC_LISTS.includes(title)) {
    ...
    // Mode zone de texte libre (Mes Repas + toutes les nouvelles listes)
    const modalTitle = document.getElementById('mealModalTitle') || document.getElementById('modalTitle');
    if (modalTitle) modalTitle.innerText = title;

    const textarea = document.getElementById('mealFreeText');
    const content = appData.items[title];
    
    // Si d'anciennes données sous forme de tableau existaient, conversion propre en texte
...
/* Enregistrement automatique du texte libre */
...
/* Fonctions pour le mode classique (champ de saisie et bouton +) */
