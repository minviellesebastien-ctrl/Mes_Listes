...
function addDiverseList() {
    ...
    if (name) {
        ...
        if (!appData.items[name]) {
            appData.items[name] = ''; /* Nouvelle liste en texte libre */
        }
        input.value = '';
        renderDiverseLists();
    }
...
/* Redirection selon le type de liste */
function openModal(title) {
    currentActiveList = title;

    // Si c'est une liste fixe standard -> Pop-up classique avec éléments
    const standardLists = ['Liste Cadeaux', 'Liste Idées', 'Liste Courses'];
    if (standardLists.includes(title)) {
    ...
    // Sinon ("Mes Repas" ou toute nouvelle liste créée) -> Pop-up texte libre
    ...
    // Si la donnée était un tableau par le passé, conversion propre en texte
...
/* Sauvegarde texte libre (Mes Repas + nouvelles listes) */
...
/* Gestion des éléments unitaires (Cadeaux, Idées, Courses) */
