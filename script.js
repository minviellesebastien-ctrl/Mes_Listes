...
/* Listes avec champ de saisie + bouton + vert */
...
function addDiverseList() {
    ...
    if (name) {
        ...
        if (!appData.items[name]) {
            appData.items[name] = ''; /* Texte libre pour les nouvelles listes */
        }
        input.value = '';
        renderDiverseLists();
    }
...
function openModal(title) {
    ...
    // 1. POP-UP CLASSIQUE (Champ + bouton +)
    ...
    // 2. POP-UP TEXTE LIBRE ("Mes Repas" & Nouvelles listes)
    // On met à jour le titre dynamique du modal texte libre
    const mealTitleEl = document.getElementById('mealModalTitle');
    if (mealTitleEl) {
        mealTitleEl.innerText = title;
    }
...
/* Enregistrement automatique de la zone de texte */
...
/* Fonctions pour les listes classiques (Courses, Idées, À Faire, Cadeaux) */
